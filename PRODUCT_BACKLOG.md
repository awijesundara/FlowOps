...
6. ~~UI/UX overhaul: admin runbook deletion, escalation/incident visual states, AND/OR dependency gates, a stakeholder monitor view, and a fixed-sidebar/reveal-animation visual pass matching ServiceOps.~~ Done 2026-09-11:
   - **Admin runbook deletion**: `DELETE /api/runbooks/{id}` (`admin:access`-gated, rejects while `status=='live'`, cascades tasks/streams/comments via existing FK constraints, deliberately preserves audit rows referencing the deleted runbook id since `audit.runbook_id` carries no FK). Red "🗑 Delete" button in the runbook detail action bar, admin-only, confirm-prompted. Tests: `test_admin_can_delete_a_runbook_but_not_while_live_and_keeps_its_audit_trail`, `test_non_admin_cannot_delete_a_runbook`.
   - **Real UI bugs found and fixed via live screenshot inspection, not assumed**: (a) the primary nav sidebar used `position:sticky`, which — while not literally broken in this session's tests — doesn't match ServiceOps's `position:fixed` sidebar and is the documented source of the "drifting border" feel on real hardware/scroll physics; switched to `position:fixed` plus `html{scrollbar-gutter:stable}` to eliminate any scrollbar-appearance layout shift, matching ServiceOps's own layout technique exactly. (b) Every modal's close button (`.close`, `.close-task`, `.close-profile`, `.close-user`, `.close-dashboard-customize`) was stretching to the full height of the modal header because flex's default `align-items:stretch` applied to an unsized `<button>` — a real, ugly, reproducible bug across all five dialogs, fixed with `align-items:flex-start` on `.modalhead` plus fixed 32×32 square dimensions. Tests: `test_sidebar_stays_fixed_in_place_while_the_page_scrolls`, `test_modal_close_buttons_are_square_icon_buttons_not_stretched`.
   - **ServiceOps-style reveal/fade polish, ported app-wide, not just admin**: a `view-reveal` keyframe (opacity+translateY) now plays on every `.view.active` switch (Home/Runbooks/Templates/Analytics/Admin/Detail) and on every `.admin-pane.active` switch inside Administration — previously only Administration's sub-panels had this treatment in the design this was modeled on; FlowOps now has it everywhere. Modal dialogs also gained a matching open animation. All animations respect `prefers-reduced-motion`. Test: `test_view_switches_and_admin_pane_switches_use_a_real_reveal_animation`.
   - **Task execution visual states**: `tasks.escalated`/`escalation_reason` and `tasks.incident`/`incident_reason` columns; `POST /api/tasks/{id}/escalate` and `/incident` (audited as `task.escalated`/`task.escalation_cleared`/`task.incident_flagged`/`task.incident_cleared`). Distinct visual treatment per state: a pulsing amber left-border + "⚠ Escalated" chip, a dark-red left-border + "🔥 Incident" chip, a stronger orange pulse for tasks running past their planned duration ("⏱ Running long", computed client-side from `started_at`+`duration`), and a clearer blocked treatment (border + "⛔" prefix) replacing the previous plain-opacity dimming. Tests: `test_task_escalation_and_incident_flags_toggle_and_are_audited`, `test_escalating_and_flagging_a_task_applies_real_visual_state_via_the_ui`.
   - **Cutover-style AND/OR dependency gates**: new `tasks.dependency_logic` column (`'and'`/`'or'`, default `'and'` — behavior-preserving for every existing runbook). The blocked-status computation and the `earliest_start()` scheduler both now respect it: an OR-gated task with 2+ dependencies unblocks (and can be scheduled) as soon as the *earliest* branch finishes, not all of them — confirmed via a dedicated test built as the mirror image of the existing AND-gated diamond-dependency test. Small badges (▶ entry, → sequential, AND, OR) render on both the timeline and node-map views. Tests: `test_or_gated_join_unblocks_on_the_first_finished_branch_not_all`, `test_dependency_gate_badges_show_entry_sequential_and_and_or_correctly`.
   - **Stakeholder monitor view**: a new standalone, unauthenticated-redirects-to-sign-in, kiosk-style page (`static/monitor.html`/`static/monitor.js`, served at `/monitor.html?rid={id}`) — dark, large-font, no app chrome, designed for a shared screen in a war room. Shows progress/task-count/stream-count/escalation-incident-count metrics and a card per task colored by status/escalation/incident/running-long, live-updating via the existing `/api/events` SSE feed (no new backend surface — it's a thin client over the same authenticated `/api/runbooks/{id}` endpoint every other view already uses). A "🖥 Monitor view" button in the runbook detail opens it in a new tab. Tests: `test_monitor_page_serves_and_real_runbook_data_flows_through_the_real_api`, `test_monitor_view_opens_from_the_runbook_and_shows_live_task_state`, `test_monitor_view_prompts_sign_in_when_not_authenticated`.
   - All 120 backend + 18 browser tests pass; verified live in the Docker preview (runbook create→delete round trip via real HTTP, `/monitor.html` and `/monitor.js` serve correctly).
7. ~~Operational-density and responsive-shell refinement.~~ Done 2026-09-12 in v0.4.9:
   - KPI and content panels now use content-driven heights with compact shared spacing, while KPI rows retain equal-height alignment. The dashboard Today panel no longer stretches to match the activity panel.
   - Runbook detail uses an approximately 80/20 execution-plan/right-rail split on wide screens, shifts to 75/25 at intermediate desktop widths, and stacks the rail below the plan at 1024px and narrower. The timing summary is a compact status strip and task secondary actions have reduced visual weight.
   - The More menu is now a viewport-aware fixed popover with right-edge alignment, vertical collision flipping, a 70vh maximum height, internal scrolling, keyboard-safe dismissal, logical grouping, and a separated destructive action. Start execution remains contextual primary while New runbook becomes secondary on detail.
   - The sidebar supports a persisted compact mode. Its former large identity, acting-role and sign-out blocks are replaced by one compact, keyboard-operable profile control with a viewport-aware account menu. A persistent acting-role indicator appears only when an acting role actually differs from the assigned role.
   - Responsive and accessibility regressions cover 1536, 1440, 1280, 1024, 768, 414, 390, 375, 320 CSS-pixel widths, menu viewport bounds, sidebar collapse and keyboard navigation, touch-target sizing, horizontal overflow, and serious/critical axe violations.
   - Dashboard correctness refinement completed 2026-09-13: greeting boundaries use the authenticated profile timezone (morning 05:00-11:59, afternoon 12:00-16:59, evening otherwise); KPI and activity rendering share the same loaded runbook collection; the redundant readiness donut is replaced by differentiated ready/live/attention workload counts; and an actual empty collection renders an actionable New runbook/Browse templates state.
   - Enterprise shell refinement completed 2026-09-13: all primary navigation shares one high-contrast selected state and 44px interaction targets; normal administrators no longer appear to be impersonating; the compact account menu owns identity, role, profile/settings, conditional role/workspace switch, sign out, and low-emphasis version; and Administration uses a 72px global rail plus 240px secondary navigation at large widths (64px/220px at 1024-1439px, then drawers below 1024px).
   - Current evidence: 123 functional tests pass; 24 real-Chrome browser tests pass with 2 environment-dependent skips (push subscription and axe bundle availability). Manual screenshot inspection at 1536px and 390px confirms the dashboard and Administration layouts have no horizontal overflow, clipped content, or conflicting empty state.
8. ~~Administration folding, dialog-action consistency, and useful portfolio analytics.~~ Done 2026-09-13 in v0.4.10:
   - Administration retains the space-efficient 72px global rail by default but now exposes the normal fold control, expands to the full navigation on demand, collapses again without leaving Administration, and persists this preference separately from the standard application sidebar.
   - All static and generated dialogs share equal-height, matching Cancel/primary action controls with non-overlapping responsive geometry, scroll-bounded content, keyboard focus visibility, and mobile-safe two-column footers.
   - Analytics is no longer a delay-only empty page. It now combines workspace-wide runbook totals, live execution count, aggregate task completion and schedule-risk count with lifecycle distribution, per-runbook progress/drill-through, actionable zero-data states, print output and delay CSV export. All values use the same loaded runbook collection and delay-report API as the dashboard.
